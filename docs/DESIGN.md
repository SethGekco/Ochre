...
- **`test_corpus.py`** *[built as one file, not two]* — over a corpus directory named by `OCHRE_CNC_CORPUS`, skipped with a printed `ok: skipped` when unset. Read→write must be **byte-identical** for untouched files, at the codec level *and* through a real `open_path`/`save_path` cycle. This is the only honest proof that a reimplemented format codec is correct, and it's the regression net that later makes the compiled tier safe to optimise aggressively.
  ...
  **It has now been run**, against a full RA2 + Yuri's Revenge install. What it found is recorded below under "What the real files taught us" — everything the synthetic tests missed, it missed because the files Ochre authors are cleaner than the files Westwood shipped.

  It checks two different things, and the second matters more than it first appears. Reproducing a file byte-for-byte exercises the *preservation* path, which deliberately avoids re-encoding — so on its own it says nothing about the encoder a user invokes the instant they paint one pixel. So every sprite is also **re-encoded from scratch**, with none of the original's choices, and the result must be lossless and structurally valid: 374,548 real frames through the live encoder.

  Validity is judged by `audit_shp`, re-derived from the format description rather than sharing code with the codec — a reader and a writer that agree with each other prove nothing, which is the entire reason this file exists. And because "spec-clean" is worthless if the audit cannot fail, a **negative control** corrupts a known-good file four ways (offset past EOF, rect escaping the canvas, a frame wider than its data, a bad scanline length) and requires each to be caught. That control is synthetic on purpose: an earlier version measured the audit's teeth by counting how many *real* files tripped it, which reported reassuring numbers on one directory and a meaningless zero on another. A check whose teeth depend on which folder you pointed it at is not a check.
